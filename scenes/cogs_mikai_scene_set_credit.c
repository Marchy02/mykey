        ...
        if(event.event == SetCreditSceneEventInput) {
            ...
            if(parse_ok) {
                ...
                if(success) {
                    ...
                    popup_set_callback(app->popup, cogs_mikai_scene_set_credit_popup_callback);
                    cogs_mikai_show_success_popup(app, "Credit Set!\nSaved in memory\nUse 'Write to Card'");
                } else {
                    popup_set_callback(app->popup, cogs_mikai_scene_set_credit_popup_callback);
                    cogs_mikai_show_error_popup(app, "Failed to set credit");
                }
            } else {
