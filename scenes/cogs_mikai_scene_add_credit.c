        ...
        if(event.event == AddCreditSceneEventInput) {
            ...
            if(parse_ok && cents > 0) {
                ...
                if(success) {
                    ...
                    popup_set_callback(app->popup, cogs_mikai_scene_add_credit_popup_callback);
                    cogs_mikai_show_success_popup(app, "Credit Added!\nSaved in memory\nUse 'Write to Card'");
                } else {
                    popup_set_callback(app->popup, cogs_mikai_scene_add_credit_popup_callback);
                    cogs_mikai_show_error_popup(app, "Failed to add credit");
                }
            } else {
