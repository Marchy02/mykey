...
// Read one line from a char buffer; advances the pointer past the newline
    ...
    if(dialog_file_browser_show(app->dialogs, file_path, file_path, &browser_options)) {
        ...
        if(storage_file_open(file, furi_string_get_cstr(file_path), FSAM_READ, FSOM_OPEN_EXISTING)) {
            ...
            if(success) {
                ...
                popup_set_callback(app->popup, cogs_mikai_scene_load_file_popup_callback);
                cogs_mikai_show_success_popup(app, "Success!\nCard loaded from file");
            } else {
                popup_set_callback(app->popup, cogs_mikai_scene_load_file_popup_callback);
                cogs_mikai_show_error_popup(app, "Invalid file format");
            }
        } else {
            popup_set_callback(app->popup, cogs_mikai_scene_load_file_popup_callback);
            cogs_mikai_show_error_popup(app, "Failed to open file");
        }
